...
namespace Infrastructure.MessageBus.PipeObservers
{
    ...
    public class LoggingReceiveObserver : IReceiveObserver
    {
        ...
        {
            await Task.Yield();
            Log.Debug("{Message} message from {Namespace} was consumed by {ConsumerType}, Duration: {Duration}s, CorrelationId: {CorrelationId}", typeof(T).Name, typeof(T).Namespace, consumerType, duration.TotalSeconds, context.CorrelationId ?? new());
        }
        ...
        {
            await Task.Yield();
            Log.Error("Fault on consuming message {Message} from {Namespace} by {ConsumerType}, Duration: {Duration}s, Error: {Error}, CorrelationId: {CorrelationId}", typeof(T).Name, typeof(T).Namespace, consumerType, duration.TotalSeconds, exception.Message, context.CorrelationId ?? new());
        }

        public async Task ReceiveFault(ReceiveContext context, Exception exception)
        {
            await Task.Yield();
            Log.Error("Fault on receiving message from exchange {Exchange}, Redelivered: {Redelivered}, Error: {Error}, CorrelationId: {CorrelationId}", context.TransportHeaders.Get<string>(ExchangeKey), context.Redelivered, exception.Message, context.GetCorrelationId() ?? new());
        }
    }
}
